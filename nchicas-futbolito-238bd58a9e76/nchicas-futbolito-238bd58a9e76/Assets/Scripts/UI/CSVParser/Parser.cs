...
namespace CSVParser
{
	class Parser
	{
		...
		public static List<string> ReadLine(string input, bool trim)
		{
			int startIndex = 0;
			int stopIndex = 0;
		...
		public static T ReadLine<T>(List<string> names, string input, bool trim) where T : new()
		{
			return CSVParser.AnonymousParser.ParseData<T>(names, ReadLine(input, trim));
		}

		public static List<List<string>> ReadStream(TextReader input, bool trim)
		{
		...
		public static List<T> ReadStream<T>(TextReader input, bool trim) where T : new()
		{
			List<T> R = new List<T>();
			string line;
